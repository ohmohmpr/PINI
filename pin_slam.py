...
def run_pin_slam(config_path=None, dataset_name=None, sequence_name=None, seed=None):
    ...
    # LIO_EKF
    LIOPara = LIOEKF_pybind._LIOPara()
    ####################################################################################################
    D2R = (np.pi / 180.0)
    R2D = (180.0 / np.pi)
    ...
    LIOPara.initstate_std.pos = np.array([0.01, 0.01, 0.01])
    LIOPara.initstate_std.vel = np.array([0.05, 0.05, 0.05])
    LIOPara.initstate_std.euler = np.array([0.1, 0.1, 0.1])

    LIOPara.initstate_std.euler = LIOPara.initstate_std.euler * D2R

    Trans_lidar_imu_origin = np.identity(4)
    Trans_lidar_imu_origin[:3, :3] = extrinsic_R
    Trans_lidar_imu_origin[:3, 3] = extrinsic_T
    LIOPara.Trans_lidar_imu_origin = Trans_lidar_imu_origin

    LIOPara.imu_tran_R = imu_tran_R
    ...
    LIOPara.Trans_lidar_imu = Trans_lidar_imu

    LIOPara.imunoise.angle_randomwalk = np.array([2, 2, 2]) * (D2R / 60.0)
    LIOPara.imunoise.velocity_randomwalk = np.array([40, 40, 40]) / 60.0
    LIOPara.imunoise.gyrbias_std = np.array([200.0, 200.0, 200.0]) * (D2R / 3600.0)
    LIOPara.imunoise.accbias_std = np.array([500.0, 500.0, 500.0]) * 1e-5
    LIOPara.imunoise.correlation_time = 1.0 * 3600
