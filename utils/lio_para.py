...
class LIO_Parameters:
    ...
    def init(self):
        ...
        idx = 0
        self.LIOPara.topic = self.config['sensor_types']['imu'][idx]["topic"]
        # depends on the topic
        for i in range(len(self.config['sensor_types']['imu'])):
            self.imu_tran_R = np.reshape(self.config['sensor_types']['imu'][idx]['imu_tran_R'], (3, 3))

            extrinsic_main_imu = np.reshape(
                self.config['sensor_types']['imu'][idx]['extrinsic_main_imu'], (4, 4))
            # print("IMU TOPIC", self.LIOPara.topic)
